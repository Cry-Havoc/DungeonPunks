...
public class PlayerCharacter : MonoBehaviour
{
    ...
    public static string GetStatusSymbols(PlayerCharacter pc)
    {
        StringBuilder sb = new StringBuilder();

        if (pc.advantageCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.positiveText)}>⛤</color>");

        if (pc.advantageAttackCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.positiveText)}>⏃</color>");
         
        if (pc.advantageDefenseCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.positiveText)}>⏣</color>");
           
        if (pc.disadvantageCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.negativeText)}>⛤</color>");
         
        if (pc.disadvantageAttackCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.negativeText)}>⏃</color>");


        if (pc.disadvantageDefenseCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.negativeText)}>⏣</color>");
         
        if (pc.exhaustionDamageLevel > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.negativeText)}>⛻</color>"); 
