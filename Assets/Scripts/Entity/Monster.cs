...
public class Monster : MonoBehaviour
{
    ...
    public string GetMonsterStatusEffect()
    {
        StringBuilder sb = new StringBuilder();

         
        if (disadvantageWhenDefendedAgainstCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.positiveText)}>⏃</color>");

        if (disadvantageWhenAttackedCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.positiveText)}>⏣</color>");
          
        if (advantageWhenDefendedAgainstCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.negativeText)}>⏃</color>");
         
        if (advantageWhenAttackedCount > 0)
            sb.Append($"<color=#{ColorUtility.ToHtmlStringRGB(GameUIManager.Instance.negativeText)}>⏣</color>");
